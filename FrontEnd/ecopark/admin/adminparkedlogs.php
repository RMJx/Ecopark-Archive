...
$url = "https://rjohnston80.webhosting3.eeecs.qub.ac.uk/adminapi/?function=parkedlogs&location=" . str_replace(' ', '%20', $_REQUEST['location']) . "&adminusername=" . $_SESSION['adminusername'] . "&API-KEY=" . $_SESSION['APIKey'];
$curl = curl_init($url);
...
<main class="l-main">
    <div class="content-wrapper content-wrapper--with-bg">
        <div class="page-content">Please find the parking history for <?php echo $_REQUEST['location']; ?> below.</em>
            <?php $total = 0;
          if (is_array($locationlogs) == 0) { echo "<h4>There is no currently parked cars for this location, once parking occurs here you will see all parked cars here for this location. </h4>"; }
          else {
          echo "<table><tr> <th>Parking ID</th><th>Registration</th><th>Username</th><th>Time Entered</th></tr>";
          for ($x = 0; $x <= sizeof($locationlogs)-1; $x++) {
            $reg = strtoupper($locationlogs[$x]['id']);
            $parkingID = $locationlogs[$x]['parkingID'];
            echo "<tr>";
            echo "<td>" . $parkingID . "</td>";
            echo "<td>" . $reg . "</td>";
            echo "<td>" . $locationlogs[$x]['userName'] . "</td>";
